"""
Preprocesamiento de comentarios de YouTube optimizado para embeddings.
...
INPUT_FILE = "comentarios_multivideo_robusto2.csv"   # CSV del scrapping
OUTPUT_FILE = "comentarios_para_embeddings.csv"      # CSV de salida

TEXT_COL = "texto"               # columna con el comentario original
STATUS_COL = "estado_video"      # columna con el estado de la descarga
...
def limpiar_texto_para_embeddings(texto):
    ...
    - Elimina URLs.
    - Elimina marcas de tiempo tipo 1:23 o 12:03:45.
    - Normaliza espacios.
    - Pasa a minúsculas.
...
def aplicar_filtros_de_calidad(df):
    ...
    """
    # 1. Filtrar solo vídeos descargados con éxito
    antes = len(df)
    ...
    if USAR_FILTRO_IDIOMA:
        print("Detectando idioma y filtrando solo comentarios en español...")
        df["lang"] = df["texto_clean"].apply(detectar_idioma_seguro)
        antes = len(df)
        df = df[df["lang"] == IDIOMA_OBJETIVO].copy()
        df = df.reset_index(drop=True)
        print(f"Comentarios en '{IDIOMA_OBJETIVO}': {len(df)} (descartadas {antes - len(df)})")
    else:
...
def main():
    ...
    print("¡Listo! Corpus optimizado para embeddings guardado.")
